...
export default function SamplingParams({isStreaming, setIsStreaming, isReasoning, setIsReasoning}) {
  return (
    <div className="w-64 border-l border-gray-200 p-4 flex flex-col gap-6 overflow-y-auto">
      ...
      {/* Streaming Toggle */}
      <ToggleButton label="Stream" paramName="stream" value={isStreaming} onChange={setIsStreaming}/>
      {/* Reasoning Toggle */}
      <ToggleButton label="Reason" paramName="reason" value={isReasoning} onChange={setIsReasoning}/>
    </div>
  );
}